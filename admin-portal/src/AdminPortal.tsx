...
import HotelWorkspace from "../../shared/operations/HotelWorkspace";
import { useHotelPrinter } from "../../shared/operations/useHotelPrinter";
import { useHotelStore } from "../../shared/operations/hotel-store";
import { BusinessDayPanel } from "../../shared/operations/BusinessDayPanel";
...
type AdminView = "dashboard" | "reports" | "hotels" | "day" | "expenses" | "catalog" | "customers" | "imports" | "requests" | "team" | "settings" | "platform";
type Locale = "en" | "ar";
...
const navigation: Array<{ id: AdminView; label: string; labelAr: string; icon: string; superOnly?: boolean }> = [
  ...
  { id: "reports", label: "Walk-in sales", labelAr: "مبيعات العملاء المباشرين", icon: "▥" },
  { id: "hotels", label: "Hotels & monthly invoices", labelAr: "الفنادق والفواتير الشهرية", icon: "▤" },
  { id: "day", label: "Daily opening & closing", labelAr: "الافتتاح والإغلاق اليومي", icon: "◷" },
  ...
  { id: "requests", label: "Approval inbox", labelAr: "طلبات الموافقة", icon: "✓" },
...
export default function AdminPortal() {
  ...
  async function openNotification(notification: AdminNotification) {
    ...
    if (notification.resourceType === "hotel" || notification.resourceType === "hotel_invoice") changeView("hotels");
    if (notification.resourceType === "business_day") changeView("day");
  ...
  if (data.user.role !== "admin") {
    return <main className="admin-denied" dir={locale === "ar" ? "rtl" : "ltr"}><div><b>{tr(locale, "Admin access required", "مطلوب صلاحية مسؤول")}</b><p>{tr(locale, "Your office account cannot open this portal.", "حساب المكتب لا يملك صلاحية فتح هذه البوابة.")}</p><Link href={process.env.NEXT_PUBLIC_OFFICE_URL ?? "http://localhost:3000"}>{tr(locale, "Return to office portal", "العودة إلى بوابة المكتب")}</Link></div></main>;
  }
  ...
  return (
    <main className="admin-shell" dir={locale === "ar" ? "rtl" : "ltr"} lang={locale}>
      ...
      <aside className={`admin-nav no-print ${mobileNavOpen ? "mobile-open" : ""}`}>
        ...
        <Link className="office-link" href={process.env.NEXT_PUBLIC_OFFICE_URL ?? "http://localhost:3000"}>↗ {tr(locale, "Open office portal", "فتح بوابة المكتب")}</Link>
      ...
      <section className="admin-main">
        ...
        {activeView === "dashboard" && <DashboardView locale={locale} />}
        ...
        {activeView === "hotels" && <HotelWorkspace data={data} locale={locale} onChanged={load} onPrintInvoice={printHotelInvoice} />}
        {activeView === "day" && <><DailyClosingReport locale={locale} /><details className="admin-shop-day no-print" open><summary>{tr(locale,"Shop day controls","أدوات يوم العمل")}</summary><BusinessDayPanel locale={locale} refreshKey={data} onChange={() => void load()} /></details></>}
        ...
        {activeView === "team" && data.admin && <TeamAdmin users={data.admin.users} grants={data.admin.grants} debts={data.admin.staffDebts} refresh={load} locale={locale} />}
        {activeView === "settings" && <><SettingsAdmin shop={data.shop} refresh={load} /><MaintenanceControl /></>}
...
function AdminLogin({ onLogin, error, locale, changeLocale }: { onLogin: (user: User) => Promise<void>; error: string; locale: Locale; changeLocale: (locale: Locale) => void }) {
  ...
  return <main className="admin-login" dir={locale === "ar" ? "rtl" : "ltr"}><form onSubmit={submit}><div className="login-language"><button type="button" onClick={() => changeLocale(locale === "en" ? "ar" : "en")}>{locale === "en" ? "العربية" : "English"}</button></div><div className="admin-login-mark">PL</div><p>{tr(locale, "ADMIN PORTAL", "بوابة الإدارة")}</p><h1>{tr(locale, "Manage your laundry business", "إدارة أعمال المغسلة بسهولة")}</h1><span>{tr(locale, "Owners and authorized technical administrators only.", "للمالك ومسؤولي التقنية المصرح لهم فقط.")}</span>{message && <div>{message}</div>}<label>{tr(locale, "Username", "اسم المستخدم")}<input value={username} onChange={(event) => setUsername(event.target.value)} required /></label><label>{tr(locale, "Password", "كلمة المرور")}<span className="password-field"><input type={showPassword ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} required /><PasswordToggle shown={showPassword} onToggle={() => setShowPassword(shown => !shown)} locale={locale} /></span></label><button disabled={busy}>{busy ? tr(locale, "Signing in…", "جارٍ تسجيل الدخول…") : tr(locale, "Sign in", "تسجيل الدخول")}</button><div className="login-help"><button type="button" onClick={() => setMessage(tr(locale,"Admins reset passwords through another authorized admin or super-admin.","يتم تغيير كلمة مرور المسؤول بواسطة مسؤول آخر أو المسؤول التقني."))}>{tr(locale,"Reset password","إعادة كلمة المرور")}</button><button type="button" onClick={() => setMessage(tr(locale,"Contact your super-admin if you cannot sign in.","تواصل مع المسؤول التقني إذا تعذر تسجيل الدخول."))}>{tr(locale,"Forgot password?","نسيت كلمة المرور؟")}</button></div><Link href={process.env.NEXT_PUBLIC_OFFICE_URL ?? "http://localhost:3000"}>{tr(locale, "Return to office portal", "العودة إلى بوابة المكتب")}</Link></form></main>;
}

function ApprovalInbox({ permissionRequests, passwordRequests, refresh, locale }: { permissionRequests: PermissionRequest[]; passwordRequests: PasswordResetRequest[]; refresh: () => Promise<void>; locale: Locale }) {
  const [temporaryPasswords, setTemporaryPasswords] = useState<Record<number, string>>({});
  const [message, setMessage] = useState("");
  async function decidePermission(id: number, status: "approved" | "denied" | "revoked") {
    await api("/api/permissions", { method: "PATCH", body: JSON.stringify({ id, status }) });
    await refresh();
  }
  async function completePassword(item: PasswordResetRequest, denied = false) {
    try {
      await api("/api/password-requests", { method: "PATCH", body: JSON.stringify({ id: item.id, status: denied ? "denied" : "completed", password: temporaryPasswords[item.id] }) });
      setTemporaryPasswords((current) => ({ ...current, [item.id]: "" }));
      await refresh();
    } catch (caught) { setMessage(caught instanceof Error ? caught.message : "Request failed"); }
  }
  return <div className="admin-page"><header className="admin-page-title"><div><p>{tr(locale,"APPROVAL INBOX","صندوق الموافقات")}</p><h1>{tr(locale,"Staff requests","طلبات الموظفين")}</h1><span>{tr(locale,"Approve only the exact date, order, expense, or password task requested.","وافق فقط على التاريخ أو الطلب أو المصروف أو مهمة كلمة المرور المطلوبة.")}</span></div></header>{message && <div className="admin-alert">{message}</div>}<section className="approval-section"><header><h2>{tr(locale,"Task permissions","صلاحيات المهام")}</h2><span>{permissionRequests.filter((item) => item.status === "pending").length} {tr(locale,"pending","معلق")}</span></header><div className="approval-list">{permissionRequests.map((item) => <article key={item.id}><div><strong>{item.staffName}</strong><span>{item.task.replaceAll("_", " ")} · {item.resourceType.replaceAll("_", " ")} · {item.resourceId}</span><small>{item.reason || tr(locale,"No reason supplied","لم يذكر سبب")}</small></div><i className={item.status}>{item.status}</i>{item.status === "pending" && <div className="approval-actions"><button className="approve" onClick={() => decidePermission(item.id,"approved")}>{tr(locale,"Approve 24h","موافقة 24 ساعة")}</button><button onClick={() => decidePermission(item.id,"denied")}>{tr(locale,"Deny","رفض")}</button></div>}{item.status === "approved" && <button onClick={() => decidePermission(item.id,"revoked")}>{tr(locale,"Revoke","إلغاء الصلاحية")}</button>}</article>)}</div></section><section className="approval-section"><header><h2>{tr(locale,"Staff password resets","إعادة كلمات مرور الموظفين")}</h2><span>{passwordRequests.filter((item) => item.status === "pending").length} {tr(locale,"pending","معلق")}</span></header><div className="approval-list">{passwordRequests.map((item) => <article key={item.id}><div><strong>{item.displayName}</strong><span>@{item.username}</span><small>{item.requestedAt}</small></div><i className={item.status}>{item.status}</i>{item.status === "pending" && <div className="password-reset-action"><input type="password" minLength={8} placeholder={tr(locale,"New temporary password","كلمة مرور مؤقتة جديدة")} value={temporaryPasswords[item.id] ?? ""} onChange={(event) => setTemporaryPasswords((current) => ({ ...current, [item.id]: event.target.value }))} /><button className="approve" onClick={() => completePassword(item)}>{tr(locale,"Set password","تعيين كلمة المرور")}</button><button onClick={() => completePassword(item,true)}>{tr(locale,"Deny","رفض")}</button></div>}</article>)}</div></section></div>;
}

// Retained temporarily as a migration reference for the scoped dashboard.

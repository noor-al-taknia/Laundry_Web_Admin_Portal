...
async function forward(request: Request, context: { params: Promise<{ path: string[] }> }) {
  ...
  const headers = new Headers({"x-laundry-portal":"admin",origin:target.origin});
  const cookie = request.headers.get("cookie")?.split(';').map(part=>part.trim()).find(part=>part.startsWith('laundry_admin_session='));

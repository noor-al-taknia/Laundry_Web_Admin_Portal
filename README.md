# Pearl Laundry — Admin portal

Owner dashboard, reports/exports, hotel customer and invoice history, catalog/customer/staff CRUD, access toggles, approval requests, notifications and shop settings. Responsive desktop/tablet/mobile layout and English/Arabic.

The portal runs at its own root URL and proxies API calls with an admin-scoped HttpOnly cookie. Office and admin can remain signed in simultaneously. Server-side roles and per-user policies remain authoritative. Shared hotel/day UI is located in shared/operations; it does not depend on a sibling checkout.

The original database keeps existing logins. For a brand-new backend, the owner username and initial password are generated in that backend's ignored platform-api/.dev.vars file. Change the initial password after signing in.
